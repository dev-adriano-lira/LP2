...
public class ControlerFornecedor {
    ...
    /**
     * Adiciona um combo a partir do nome do fornecedor, nome do combo, descricao do combo, fator e os produtos do
     * combo, alem de verifica se os atributos existem, se nao sao vazios ou nulos, e se um combo nao possui um
     * combo na lista de produtos, caso contrario lanca uma excessao. Alem disso calcula o preco final.
     *
     * @param fornecedor     para o nome do fornecedor.
     * @param nomeCombo      para o nome do combo.
     * @param descricaoCombo para a descricao do combo.
     * @param fator          para o fator.
     * @param produtos       para os produtos do combo.
     */
    public void adicionaCombo(String fornecedor, String nomeCombo, String descricaoCombo, double fator, String produtos) {
        Util.testaNulleVazio(fornecedor, "Erro no cadastro de combo: fornecedor nao pode ser vazio ou nulo.");
        Util.testaNulleVazio(nomeCombo, "Erro no cadastro de combo: nome nao pode ser vazio ou nulo.");
        Util.testaNulleVazio(descricaoCombo, "Erro no cadastro de combo: descricao nao pode ser vazia ou nula.");
        Util.testaFator(fator, "Erro no cadastro de combo: fator invalido.");
        Util.testaNulleVazio(produtos, "Erro no cadastro de combo: combo deve ter produtos.");
        ...
        double precoIntegral = 0;
        String[] prods = produtos.split(",");
        for (String prod : prods) {
            String[] dados = prod.split(" - ");
            Produto produto = this.fornecedores.get(fornecedor).consultaProduto(dados[0].trim(), dados[1].trim());
            Util.testaObjetoNull(produto, "Erro no cadastro de combo: produto nao existe.");
            precoIntegral += this.fornecedores.get(fornecedor).getPrecoProduto(dados[0].trim(), dados[1].trim());
        }
        double precoFinal = precoIntegral - (precoIntegral * fator);
        this.fornecedores.get(fornecedor).adicionaCombo(nomeCombo, descricaoCombo, precoFinal, precoIntegral, fator);
    }

    /**
     * Edita um combo a partir do nome do combo, descricao do combo, do nome do fornecedor e do novo fator
     * a ser editado.
     *
     * @param nomeCombo      para o nome do combo.
     * @param descricaoCombo para a descricao do combo.
     * @param fornecedor     para o nome do fornecedor.
     * @param novoFator      para o novo fator a ser editado.
     */
    public void editaCombo(String nomeCombo, String descricaoCombo, String fornecedor, double novoFator) {
        Util.testaNulleVazio(fornecedor, "Erro na edicao de combo: fornecedor nao pode ser vazio ou nulo.");
        Util.testaNulleVazio(nomeCombo, "Erro na edicao de combo: nome nao pode ser vazio ou nulo.");
        Util.testaNulleVazio(descricaoCombo, "Erro na edicao de combo: descricao nao pode ser vazia ou nula.");
